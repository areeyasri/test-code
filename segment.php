...
class Segment {
    ...
    private function _segment_by_dictionary_reverse($input_array) {
	 ...
	 private function _segment_by_dictionary_rude($input_array) {

        $result_array_rude = array();
        $rude_string = '';
        ...
        while ($pointer <= $length_of_string) {
            ...
            $rude_string .= $input_array[$pointer];

            if (isset($this->_dictionary_array_rude[crc32($rude_string)])) { // ถ้าเจอใน Dict //
                $dup_array = array();
                $dup_array[] = array(
                    'title' => $rude_string,
                    'to_mark' => $pointer + 1,
                ...
                for ($i = $pointer + 1; $i <= $length_of_string; $i++) {
                    ...
                    //echo $more_tmp.'<br/>';
                    //echo $more_tmp.'<br/>';
                    if (isset($this->_dictionary_array_rude[crc32($more_tmp)])) {
                        $dup_array[] = array(
                            'title' => $more_tmp,
                            'to_mark' => $i + 1,
                        );
                ...
                if (count($dup_array) > 0) {
                    $result_array_rude[] = $dup_array[count($dup_array) - 1]['title'];
        ...
        if ($rude_string != '') { //  ส่วนที่เหลือ ถ้าไม่เจอใน Dict
            $result_array_rude[] = $rude_string;
        }

        if (count($result_array_rude) == 0) {
            return array(implode($input_array));
        }
		
        return $result_array_rude;
    }
	...
	public function get_rude_word($input_array){
		...
		$result_rude = array_fill( 0, count($this->_dictionary_array_rude)-1, null);
		...
		$result_return = array_fill( 0, $i-1, null);
		
		$i = 0;
		foreach($this->_dictionary_array_rude as $eachrude){
			if($result_rude[$i] == 1)
				$result_return[$i] = $eachrude;
			$i = $i+1;
		}
		//$this->_result_return = $result_return;
